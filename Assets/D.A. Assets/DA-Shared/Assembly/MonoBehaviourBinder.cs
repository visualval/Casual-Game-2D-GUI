...
namespace DA_Assets.Shared
{
    ...
    public static class MonoBehaviourBinderExtensions
    {
        public static T SetMonoBehaviour<T, T3>(this T type, T3 monoBeh) where T3 : MonoBehaviour where T : MonoBehaviourBinder<T3>
        {
            if (type == null)
            {
                AttributeValidator.Validate<T>(typeof(SerializableAttribute));
    ...
    public static class AttributeValidator
    {
            ...
            where T : class
        {
            ...
            foreach (Type att in atts)
            {
                ...
                var attribute = Attribute.GetCustomAttribute(type, att);

                if (attribute != null)
                {
                    //Debug.Log($"Attribute '{att.Name}' is applied to '{type.Name}'.");
                }
                else
                {
